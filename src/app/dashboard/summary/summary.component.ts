...
import {  Chart as ChartJS, CategoryScale, LinearScale, BarElement, Title, Tooltip, Legend, ArcElement, LineElement, Filler, Chart } from 'chart.js';
...
export class SummaryComponent {
  ...
  calculateStats(): void {
    ...
    this.rentalProperties = this.properties.filter(property => property.property_type === 'rental').length;
    this.saleProperties = this.properties.filter(property => property.property_type === 'sale').length;
    this.activeProperties = this.properties.filter(property => property.status === true).length;
    this.inactiveProperties = this.properties.filter(property => property.status === false).length;
    
    const uniqueAgents = new Set(this.properties.map(property => property.agent.id));
    this.totalAgents = uniqueAgents.size;

    const totalRatings = this.properties.reduce((sum, property) => sum + property.rating, 0);
    this.averageRating = totalRatings / this.totalProperties;
  }
  createCharts() {
    ChartJS.register(
      CategoryScale,
    ...
    // Active vs. Inactive Properties Bar Chart
    new Chart('statusTrendChart', {
      type: 'bar',
      data: {
        labels: ['Active', 'Inactive'],
        datasets: [{
          label: 'Property Status',
          data: [45, 55], // Example data
          backgroundColor: ['rgba(75, 192, 192, 1)', 'rgba(255, 99, 132, 1)']
        }]
      }
    });

    // Rental vs. Sale Properties Pie Chart
    new Chart('propertyTypeChart', {
      type: 'pie',
      data: {
        labels: ['Rental', 'Sale'],
        datasets: [{
